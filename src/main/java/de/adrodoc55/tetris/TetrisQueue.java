...
public class TetrisQueue extends JComponent {
  ...
  public void paint(Graphics g) {
    ...
    for (int x = 0; x < length; x++) {
      ...
      for (Coordinate part : baustein.getLayout()) {
        g.fillRect(part.getX() * TetrisGrid.blockSize, part.getY() * TetrisGrid.blockSize,
            TetrisGrid.blockSize, TetrisGrid.blockSize);
      }
    }
  ...
  public Dimension getPreferredSize() {
    return new Dimension(5 * TetrisGrid.blockSize, length * 5 * TetrisGrid.blockSize);
  }
